...
- 浏览聊天列表、查看消息历史，并通过 FCM 及时接收消息通知
- 直接从手表发送文本消息、语音消息，以及图片或视频消息
- 长按消息进入选择模式，支持单选或多选后执行快捷操作
- 支持消息回复和单条/多条消息转发，支持在可用时隐藏发送者名称或移除说明文字
- 支持 Telegram VoIP 通话，可拨打和接听电话，并提供来电通知、通话状态更新和音频输出切换
- 可在消息详情中预览、下载、播放和保存支持的媒体内容
- 当前支持渲染的消息类型包括文本、图片、视频、文档、GIF/动画、动画表情、贴纸、语音消息、圆形视频消息、音频文件和通话记录
- 支持聊天列表、消息历史、消息预览、媒体相册和消息交互信息更新
